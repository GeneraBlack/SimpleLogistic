...
public class ProportionalPipeNetwork {
    ...
    private static ResourceHandler<ItemResource> getItemHandler(ServerLevel level, BlockPos pos, Direction dir) {
        return level.getCapability(Capabilities.Item.BLOCK, pos, dir);
    }

    private static ResourceHandler<FluidResource> getFluidHandler(ServerLevel level, BlockPos pos, Direction dir) {
        return level.getCapability(Capabilities.Fluid.BLOCK, pos, dir);
    }

    private static EnergyHandler getEnergyStorage(ServerLevel level, BlockPos pos, Direction dir) {
        return level.getCapability(Capabilities.Energy.BLOCK, pos, dir);
    }
    ...
    private boolean matchesItemFilter(ItemStack stack, PipeBlockEntity.PipeOperation op) {
        ...
        if (op.tagFilter != null && !op.tagFilter.isBlank()) {
            ...
            if (filter.startsWith("#")) {
                ...
                boolean matches = stack.typeHolder().tags().anyMatch(t -> t.location().toString().equalsIgnoreCase(tagLoc));
    ...
    private void processItems(List<SourceTask> sources, List<SinkTask> sinks) {
        ...
        for (SourceTask source : sources) {
            ResourceHandler<ItemResource> sourceHandler = getItemHandler(source.level(), source.machinePos(), source.dir());
            ...
            for (int slot = 0; slot < sourceHandler.size(); slot++) {
                ...
                ItemStack sampleStack = resource.toStack();
                if (!matchesItemFilter(sampleStack, source.op())) continue;

                int simExtracted;
                try (Transaction tx = Transaction.openRoot()) {
                    simExtracted = sourceHandler.extract(slot, resource, 64, tx);
                }
                if (simExtracted <= 0) continue;

                ItemStack availableStack = resource.toStack(simExtracted);
                ...
                for (List<SinkTask> priorityGroup : priorityGroups.values()) {
                    ...
                    for (SinkTask sink : priorityGroup) {
                        ...
                        ResourceHandler<ItemResource> sinkHandler = getItemHandler(sink.level(), sink.machinePos(), sink.dir());
                        if (sinkHandler != null) {
                            ItemStack simResult = ItemUtil.insertItemReturnRemaining(sinkHandler, availableStack.copy(), true, null);
                    ...
                    for (Map.Entry<SinkTask, Integer> entry : allocations.entrySet()) {
                        ResourceHandler<ItemResource> sinkHandler = getItemHandler(entry.getKey().level(), entry.getKey().machinePos(), entry.getKey().dir());
                        if (sinkHandler != null) {
                            ItemStack simResult = ItemUtil.insertItemReturnRemaining(sinkHandler, availableStack.copyWithCount(entry.getValue()), true, null);
                    ...
                    int extractedCount;
                    try (Transaction tx = Transaction.openRoot()) {
                        extractedCount = sourceHandler.extract(slot, resource, confirmedTotal, tx);
                        tx.commit();
                    }
                    ...
                    for (Map.Entry<SinkTask, Integer> entry : confirmed.entrySet()) {
                        ResourceHandler<ItemResource> sinkHandler = getItemHandler(entry.getKey().level(), entry.getKey().machinePos(), entry.getKey().dir());
                        if (sinkHandler != null) {
                            ItemStack leftover = ItemUtil.insertItemReturnRemaining(sinkHandler, extracted.copyWithCount(entry.getValue()), false, null);
                    ...
                    if (notInserted > 0) {
                        ...
                        ItemStack stillLeft = ItemUtil.insertItemReturnRemaining(sourceHandler, returnStack, false, null);
                    ...
                    int nextSim;
                    try (Transaction tx = Transaction.openRoot()) {
                        nextSim = sourceHandler.extract(slot, resource, 64, tx);
                    }
                    availableStack = nextSim > 0 ? resource.toStack(nextSim) : ItemStack.EMPTY;
                }
            }
        }
    }
    ...
    private void processFluids(List<SourceTask> sources, List<SinkTask> sinks) {
        ...
        for (SourceTask source : sources) {
            ResourceHandler<FluidResource> sourceHandler = getFluidHandler(source.level(), source.machinePos(), source.dir());
            if (sourceHandler == null) continue;
            ...
            for (int slot = 0; slot < sourceHandler.size(); slot++) {
                ...
                int availableAmount;
                try (Transaction tx = Transaction.openRoot()) {
                    availableAmount = sourceHandler.extract(slot, resource, 1000, tx);
                }
                if (availableAmount <= 0) continue;

                int remainingSupply = availableAmount;
                ...
                for (List<SinkTask> priorityGroup : priorityGroups.values()) {
                    ...
                    for (SinkTask sink : priorityGroup) {
                        ResourceHandler<FluidResource> sinkHandler = getFluidHandler(sink.level(), sink.machinePos(), sink.dir());
                        if (sinkHandler != null) {
                            int accepted;
                            try (Transaction tx = Transaction.openRoot()) {
                                accepted = sinkHandler.insert(resource, remainingSupply, tx);
                            }
                            if (accepted > 0) {
                                demands.put(sink, accepted);
                                totalDemand += accepted;
                    ...
                    for (Map.Entry<SinkTask, Integer> entry : demands.entrySet()) {
                        ...
                        if (amount > 0) {
                            ResourceHandler<FluidResource> sinkHandler = getFluidHandler(entry.getKey().level(), entry.getKey().machinePos(), entry.getKey().dir());
                            if (sinkHandler != null) {
                                int realAccept;
                                try (Transaction tx = Transaction.openRoot()) {
                                    realAccept = sinkHandler.insert(resource, amount, tx);
                                }
                                if (realAccept > 0) {
                                    confirmed.put(entry.getKey(), realAccept);
                                    confirmedTotal += realAccept;
                    ...
                    int extracted;
                    try (Transaction tx = Transaction.openRoot()) {
                        extracted = sourceHandler.extract(slot, resource, confirmedTotal, tx);
                        tx.commit();
                    }
                    ...
                    for (Map.Entry<SinkTask, Integer> entry : confirmed.entrySet()) {
                        ...
                        if (sinkHandler != null) {
                            int inserted;
                            try (Transaction tx = Transaction.openRoot()) {
                                inserted = sinkHandler.insert(resource, entry.getValue(), tx);
                                tx.commit();
                            }
                    ...
                    int notInserted = extracted - totalInserted;
                    if (notInserted > 0) {
                        try (Transaction tx = Transaction.openRoot()) {
                            sourceHandler.insert(resource, notInserted, tx);
                            tx.commit();
                        }
                    }
                }
    ...
    private void processEnergy(List<SourceTask> sources, List<SinkTask> sinks) {
        ...
        for (SourceTask source : sources) {
            EnergyHandler sourceHandler = getEnergyStorage(source.level(), source.machinePos(), source.dir());
            if (sourceHandler == null) continue;

            int remainingSupply;
            try (Transaction tx = Transaction.openRoot()) {
                remainingSupply = sourceHandler.extract(Integer.MAX_VALUE, tx);
            }
            ...
            for (List<SinkTask> priorityGroup : priorityGroups.values()) {
                ...
                for (SinkTask sink : priorityGroup) {
                    EnergyHandler sinkHandler = getEnergyStorage(sink.level(), sink.machinePos(), sink.dir());
                    if (sinkHandler != null) {
                        int accepted;
                        try (Transaction tx = Transaction.openRoot()) {
                            accepted = sinkHandler.insert(remainingSupply, tx);
                        }
                        if (accepted > 0) {
                            demands.put(sink, accepted);
                            totalDemand += accepted;
                ...
                for (Map.Entry<SinkTask, Integer> entry : demands.entrySet()) {
                    ...
                    int amount = (int) Math.floor(share * toTransfer);
                    if (amount > 0) {
                        EnergyHandler sinkHandler = getEnergyStorage(entry.getKey().level(), entry.getKey().machinePos(), entry.getKey().dir());
                        if (sinkHandler != null) {
                            int realAccept;
                            try (Transaction tx = Transaction.openRoot()) {
                                realAccept = sinkHandler.insert(amount, tx);
                            }
                            if (realAccept > 0) {
                                confirmed.put(entry.getKey(), realAccept);
                                confirmedTotal += realAccept;
                ...
                int extracted;
                try (Transaction tx = Transaction.openRoot()) {
                    extracted = sourceHandler.extract(confirmedTotal, tx);
                    tx.commit();
                }
                if (extracted <= 0) continue;
                ...
                for (Map.Entry<SinkTask, Integer> entry : confirmed.entrySet()) {
                    ...
                    if (sinkHandler != null) {
                        int received;
                        try (Transaction tx = Transaction.openRoot()) {
                            received = sinkHandler.insert(entry.getValue(), tx);
                            tx.commit();
                        }
                ...
                if (notReceived > 0) {
                    try (Transaction tx = Transaction.openRoot()) {
                        sourceHandler.insert(notReceived, tx);
                        tx.commit();
                    }
                }
